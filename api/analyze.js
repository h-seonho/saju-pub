...
export default async function handler(req, res) {
  ...
  try {
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [{
        role: "system",
        content: "당신은 30년 경력의 정통 명리학 대가입니다. 사용자 정보를 바탕으로 깊이 있고 품격 있는 사주 풀이를 작성하세요. [필수 지침] 1. 마크다운 코드 블록(```)을 사용하지 마세요. 2. '종합 사주 총평' 타이틀을 중복하지 마세요. 3. 전문적이고 진중한 어조를 사용하세요. 4. 다음 섹션 순서대로 작성하세요: 종합 사주 총평, 초년운 및 성장기, 중년운 (사회적 성취), 말년운 (재물과 안식), 직장 및 사업운 (재물 모으는 비법 포함), 대인관계 및 연애/결혼운, 건강 및 체질 조언"
      }, {
        role: "user",
